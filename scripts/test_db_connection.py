...
from adapters.database import get_session, init_db  # noqa: E402
...
async def test_connection() -> bool:
    ...
    try:
        ...
        async for client in get_session():
            ...
            if existing_tables:
                ...
                if missing:
                    ...
                    print("   Execute as migrações em migrations/ (001, 002, 003, 004)")
        ...
        if (
            ...
            print("   4. Verifique SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY no .env")
            print()
        elif (
            ...
            print("   1. A API do Supabase está temporariamente indisponível")
            print("   2. Há problemas de rede ou firewall")
            print()
            print("   ✅ Soluções:")
            print("   1. Verifique se SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY estão corretas")
            print("   2. Verifique se o projeto Supabase está ativo")
            ...
            print()
            print("   Isso geralmente significa que a API do Supabase está indisponível.")
            print("   Tente uma das seguintes soluções:")
            print()
            print("   1. Verifique se o projeto Supabase está ativo (não pausado)")
            print("   2. Verifique SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY no .env")
            print("   3. Aguarde alguns minutos e tente novamente")
            print()
        else:
            print("💡 Possíveis causas:")
            print("   1. SUPABASE_URL incorreta no arquivo .env")
            print("   2. SUPABASE_SERVICE_ROLE_KEY incorreta ou expirada")
            print("   3. Firewall bloqueando conexão")
            print("   4. Projeto Supabase pausado ou indisponível")
            print()

        print("   Verifique README.md para mais detalhes sobre configuração.")
        return False
